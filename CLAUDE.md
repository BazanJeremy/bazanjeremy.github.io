...
| Correctif d'attribution (20.09) — le seul point que T18 avait signalé sans le réécrire, **arbitré par Jérémy après le merge**. L'article ouvrait sur « le 15 septembre, […] Vals AI a publié le compte rendu d'une expérience » : or le récit vient du **fil X de Vals AI et du direct** (15.09, date confirmée par Dexerto et Tom's Hardware), alors que la page CUA-bench de vals.ai est datée « Updated 9/18/2026 » et ne contient **ni** l'anecdote **ni** la citation mémoire (the-decoder, lui, date « le rapport » du 17.09 — les médias divergent). « a publié le compte rendu » → « a partagé le récit » ; en EN « published an account » → « shared an account » (l'EN était moins exposé : « an account » n'annonçait pas un document, seul « published » posait problème). **Une ligne par fichier, sans reflux de paragraphe** : les lignes de remplacement se terminent comme les originales (`d'une` / `an`). Raison du correctif, propre à cet article : sa thèse est qu'une règle doit dire d'où elle vient et à quelle date — l'ouverture ne pouvait pas se tromper sur sa propre source. **Laissé tel quel sur décision de Jérémy** : « Astra a fini par se corriger […] L'alerte a été levée » — la ligne de log (« SUGARCANE, NOT creeper! ») corrige une *identification*, pas la règle ; choix rhétorique non garanti par la source, pas erreur factuelle, et l'article se protège déjà deux paragraphes plus haut (« une partie, un incident, aucun contrefactuel ») | ✅ mergée (#40), déployée · texte servi relu en prod dans les deux langues |
| Maintenance — dépendances (20.09, #41) — les **3 alertes Dependabot** restantes fermées par `npm audit fix` : `svgo` ×2 (GHSA-w27v-7q3p-w38r **high, CVSS 8.2** et GHSA-4vpr-x523-8j87 medium 6.1, toutes deux sur le plugin `removeScripts`) et `devalue` ×1 (GHSA-9rgm-9g3h-6x36 medium 5.3, DoS sur entrée malformée). **Aucune n'atteignait le visiteur**, mesuré : `dist/` livre 0 fichier `.js` et 0 balise `<script>` ; les deux avis `svgo` sont des **contournements de sanitizer** qui supposent un SVG venu d'un utilisateur non fiable, alors que le dépôt n'a qu'un seul SVG (`public/favicon.svg`, écrit à la main, copié verbatim sans passer par le pipeline d'assets — vérifié identique à l'octet après le bump) ; le `devalue` est un CVE de disponibilité sans entrée hostile, même forme que le `js-yaml` de #26. Motif du correctif : hygiène (une liste d'alertes bruyante cache celle qui compte), pas exposition. **`package.json` intact à l'octet** (le champ `allowScripts` survit) et, contrairement à #26, **Astro n'a pas bougé** (7.3.1). 7 paquets transitifs changent : `svgo` 4.0.2 → 4.1.0, `devalue` 5.8.1 → 5.9.4, `sax` 1.6.0 → 1.6.1, `entities` dédupliqué, et **deux majors** tirés par `svgo` — `css-select` 5.2.2 → 6.0.0, `css-what` 6.2.2 → 7.0.0. Vérif : **18/18 pages identiques à l'octet** contre la prod, hash du CSS inchangé, `sitemap-0.xml` identique, 5 assets identiques dont `favicon.svg` et les deux cartes OG, `npm audit` = 0 | ✅ mergée (#41), déployée · **0 alerte Dependabot ouverte** après le merge |
| T19 — 8e article (« Le droit à l'erreur d'une IA de détection », `anomaly-sentinel`, FR par Jérémy + traduction EN) + correctif d'une ligne de T15 (FR + EN). **Une première version a été retirée avant toute PR** : elle décrivait un LLM qui lit l'enchaînement là où les règles échouent, et un outil qui « produit un rapport d'audit ». Le code la contredisait sur le fond : le LLM reçoit **un événement à la fois** (`to_classifier_context()`, l'enchaînement résumé en `daily_tx_count` / `previous_country` / `minutes_since_last_tx`), le prompt lui donne **les cinq typologies des règles** et limite `rule_triggered` à celles-ci, et le rapport d'audit figure parmi les « pistes envisagées » du README. Jérémy a réécrit l'article (le classifieur comme système sous test), puis validé **cinq corrections** : (1) « 5 % de fausses alertes » → **taux de faux positifs** (« sur cent cas normaux, cinq signalés à tort »), sinon la phrase contredit les 85 % de précision ; (2) prompts : **aucune comparaison de performance entre versions** — chaque version doit passer le seuil absolu, seul le taux de FP est comparé (`m11 <= m10 + 0.05`, fintech seulement), et **sans clé, aucun prompt n'est lu** ; (3) sur les cinq défauts du README EN, **un seul** touche les règles ; (4) l'anecdote du capteur **rejouée** dans un clone (règle batterie déplacée après les règles SpO₂) → 1 échec, `test_scenario_detection[sensor_drift-True-medium]`, alors que le test « jamais critique » **passe** (la règle de variation renvoie `high`) → « critique » et « c'est précisément ce que vérifie… » retirés ; l'état d'avant la correction n'est pas dans l'historique public (ordre déjà corrigé dans `2c2a6d3`) ; (5) « données personnelles » → « identifiants directs » (constantes, service et médication partent au LLM). **Deux ajouts signalés**, tirés de la réponse de Jérémy « **mode LLM jamais lancé** » : « et je ne l'ai pas encore fait » dans les limites, et « le LLM **est soumis** aux mêmes tests » au lieu de « passe » (qu'on peut lire « réussit »). T15 : « C'est le terrain d'anomaly-sentinel, et là, le LLM apporte… » → attribué à sa veille + « je ne l'ai pas mesuré ». Vérifié : 182 tests collectés, 163 passés ici sans clé (les 19 UI n'ont pas été lancés). **Côté outil, écarts fermés par `anomaly-sentinel#3`** (le texte suit le code, classifieur inchangé). README FR/EN alignés : géo = changement de pays en moins de 120 min, vélocité ≥ 15, portée réelle des tests de prompts, test qui garantit vraiment l'ordre des règles capteur, « identifiants directs », aucun job CI avec clé, quality gate non bloquant faute de protection de branche. `fallback.py` supprimé (importé nulle part, seuils 20/30). Amendements datés : ADR-001 (Allure et journaux d'audit non implémentés) et ADR-002 (vélocité, clé, et **chiffres d'évaluation 99,2 / 96,8 / 88,1 % sans source dans le dépôt, « à lire comme non vérifiés »**). La vérification y a trouvé **un vrai défaut, que la suite tolérait** : le générateur `card_testing` tirait jusqu'à 1,00 € alors que la règle exige `< 1.00`. Effet mesuré : 5 manqués sur 2 000, rappel fintech jusqu'à 96 %, et `test_high_confidence_predictions_are_correct` (fintech) **instable** (64 échecs sur 5 000 rejeux de sa logique). Corrigé (0,01–0,99) → 0 / 20 000 et 0 / 5 000. **Puis `anomaly-sentinel#4`**. La règle « compte dormant » passe de « > 89 j » à « 90 j ou plus », comme le disent le README, le prompt et le générateur. C'est le seul changement de comportement : 89 j 23 h 59 min → rien, 90 j pile → alerte, 0 manqué sur 20 000 tirages, toujours 182 tests. Les écarts du prompt **medtech** `v1.0` sont documentés à côté de ceux du fintech : retard de calibration absent du contexte, batterie < 20 % contre ≤ 20 %, désaturation rapide limitée à SpO₂ < 94 %, hypoglycémie « moyenne à haute ». Une note sous le tableau des défauts dit que la ligne « Sensor drift classified as `critical` » décrit l'époque et n'est pas reproductible. **Protection de branche sur `main` activée le 22.09** (décision de Jérémy, sur conseil) : PR obligatoire (0 approbation), check `Quality gate` requis, administrateurs compris, force-push et suppression interdits. Motif : le README disait que le gate « ne bloque pas un merge », le seul endroit où le dépôt contredisait l'article (« écrit avant, et bloquant »). Vérifiée en conditions réelles sur `anomaly-sentinel#5` : `BLOCKED` pendant la CI, `CLEAN` une fois verte. README FR/EN mis à jour par #5. **Reste ouvert, décision de Jérémy** : aligner les prompts (`fintech_v1.2`, `medtech_v1.1`) attend un premier run en mode LLM, parce que le faire sans mesure contredirait le framework, et que chaque version ajoutée au test paramétré change le nombre de tests (182, publié dans l'article T19) | ✅ mergée (#43), déployée · `anomaly-sentinel#3`, `#4` et `#5` mergées le 22.09, CI verte (11 jobs, UI Playwright comprise) · prod relue le 22.09 : 20/20 pages conformes au build de `main` (6 au faux positif CRLF près), phrases corrigées présentes en FR et en EN |
...
**La ligne d'avancement qu'on ajoute dans une PR est périmée dès le merge** : elle
